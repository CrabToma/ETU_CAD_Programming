/*
  Çàäàíèå: Äàíû N òî÷åê íà ïëîñêîñòè. Íààéòè ñðåäè íèõ òî÷êè, ÿâëÿþùèåñÿ âåðøèíàìè ôèãóðû,
  ñîäåðæàùåé ìàêñèìàëüíîå ÷èñëî çàäàííûõ òî÷åê
  Àâòîð: Ôàíàñþòêèí Ðîìàí Âëàäèìèðîâè÷  Ãðóïïà: 4354 Âåðñèÿ ïðîãðàììû: K.1.24
  Äàòà íà÷àëà: 12.12.24  Çàâåðøåíèÿ: 23.12.24
*/
#include <iostream>
#include <fstream>
#include <locale>
...
void ScanInputFile(fstream& f, fstream& g, fstream& h, float A[][2], int& size)
{
    ...
    {
        h << "Êîëè÷åñòâî ýëåìåíòîâ ñ÷èòàííîå ïðîãðàììîé: " << size << "\n";
        if (size < 0)
        {
            size = 0;
            g << "Áûëè ñäåëàíû êîððåêòèðîâêè, size = 0\n";
            h << "Áûëè ñäåëàíû êîððåêòèðîâêè, size = 0\n";
        }
        if (size > n)
        {
            size = n;
            g << "Áûëè ñäåëàíû êîððåêòèðîâêè, size = n\n";
            h << "Áûëè ñäåëàíû êîððåêòèðîâêè, size = n\n";
        }
        f >> noskipws;
        ...
        for (i = 0; i < size && !f.eof();)
        {
            ...
            if (countInLine == 0)
            {
                h << inpnum << " ñòðîêà: Îòñóòñòâóåò x è y\n";
            }
            else if (countInLine == 1)
            {
                h << inpnum << " ñòðîêà: Îòñóòñòâóåò y\n";
            }
            else
            {
                h << inpnum << " ñòðîêà: Ñ÷èòàííà òî÷êà (" << A[i][0] << ", " << A[i][1] << ")\n";
                i++;
            }
        ...
        g << "Êîëè÷åñòâî ýëåìåíòîâ: " << size << "\nA = {";
        h << "Êîëè÷åñòâî ýëåìåíòîâ: " << size << "\nA = {";
            ...
            for (int d = 0; d < size; d++)
            {
                ...
                for (int p = 0; p < size; p++)
                {
                    ...
                    for (int k = 0; k < size; k++)
                    {
                        ...
                        {
                            h << "Ôèãóðû - 2 òðåóãîëüíèêà ðàâíû, íî îñíàâàíèÿ íå ïàðàëåëüíû,:";
                        }
                        else if (AreEqualTriangles(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5))
                        {
                            h << "Ôèãóðû - 2 òðåóãîëüíèêà ðàâíû, íî îñíàâàíèÿ íå ïàðàëåëüíû, È ôèãóðà íå óíèêàëüíà:";
                        }
                        else if (AreParalel(h,x1, y1, x2, y2, x3, y3, x4, y4, x5, y5)&& IsFigureUnique(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5))
                        {
                            h << "Ôèãóðû - 2 íå ðàâíûõ òðåóãîëüíèêà, íî îñíàâàíèÿ ïàðàëåëüíû:";
                        }
                        else if (AreParalel(h, x1, y1, x2, y2, x3, y3, x4, y4, x5, y5))
                        {
                            h << "Ôèãóðû - 2 íå ðàâíûõ òðåóãîëüíèêà, íî îñíàâàíèÿ ïàðàëåëüíû:, È ôèãóðà íå óíèêàëüíà";
                        }
                        else
                        {
                            h << "Ôèãóðû - 2 íå ðàâíûõ òðåóãîëüíèêà, îñíàâàíèÿ íå ïàðàëåëüíû:";
                        }
                        h << "\n(" << x1 << ", " << y1 << "), "
                            << "(" << x2 << ", " << y2 << "), "
                            << "(" << x3 << ", " << y3 << "), "
...
void IsPointInFigure(float A[][2], int size, fstream& g, fstream& h)
{
    for (int j = 0; j < FiguresCount; j++)
    {
        ...
        g << "\nÊîëè÷åñòâî òî÷åê â ôèãóðå ¹: " << j + 1 << " = " << PointsInFigure[j];
        h << "\nÊîëè÷åñòâî òî÷åê â ôèãóðå ¹: " << j + 1 << " = " << PointsInFigure[j];
    }
}
void FindMaxFigure(fstream& g, fstream& h)
{
    ...
    g << "\nÔèãóðû ñ íàèáîëüøèì êîëè÷åñòâîì òî÷åê( " << MaxCountOfPoints << " ) ëåæàùèõ âíóòðè íèõ:";
    h << "\nÔèãóðû ñ íàèáîëüøèì êîëè÷åñòâîì òî÷åê ëåæàùèõ âíóòðè íèõ:";
    for (int i = 0; i < k; i++)
    {
        h << "\nÔèãóðà ¹" << FiguresWithMaxCountofPoints[i] + 1;
        g << "\nÔèãóðà ¹" << FiguresWithMaxCountofPoints[i] + 1;
    }
}
...
void main()
{
    ...
    if (!g.is_open())
        cout << "Íå óäîëîñü îòêðûòü âûõîäíîé ôàéë results.txt";
    else
    {
        h.open("protocol.txt", ios::out);
        if (!h.is_open())
            cout << "Íå óäîëîñü îòêðûòü âûõîäíîé ôàéë protocol.txt";
        else
        {
            f.open("in.txt", ios::in);
            if (!f.is_open())
                cout << "Íå óäîëîñü îòêðûòü âõîäíîé ôàéë in.txt";
            else
            {
